    ...
    <div>
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          {last && (
            <div className={`rounded-xl border-2 p-5 flex items-start gap-4 ${last.success ? 'bg-green-50 border-green-400' : 'bg-red-50 border-red-400'}`}>
              {last.success
              ...
              <div>
                ...
                {last.holder_name && <p className="text-sm font-medium text-gray-900 mt-1">{last.holder_name}</p>}
            ...
            {history.map((h) => (
              <div key={h.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!h.result.success && <p className="text-xs text-red-600 truncate">{h.result.message}</p>}
                </div>
                <p className="text-xs text-gray-400 shrink-0">{format(h.ts, 'HH:mm:ss')}</p>
              </div>
            ))}
          </div>
        </Card>

          ...
          {pkgs.map(pkg => (
            <div key={pkg.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => deleteMut.mutate(pkg.id)} className="text-gray-400 hover:text-red-500 transition-colors">
...
function CreateEventModal({ open, onClose, venueId }: { open: boolean; onClose: () => void; venueId: string }) {
  ...
  const createMut = useMutation({
    ...
    onError: (e: AxiosError<{ detail: string }>) => {
      ...
      setErr(typeof detail === 'string' ? detail : JSON.stringify(detail) || 'Error al crear evento')
    },
  })
    ...
    <Modal open={open} onClose={onClose} title="Nuevo evento" size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Input id="ev-name" label="Nombre del evento *" value={form.name}
            onChange={e => { const name = e.target.value; setForm(f => ({ ...f, name, slug: slugify(name) })) }}
            placeholder="Bad Bunny en vivo" className="col-span-2" />
...
export default function EventsPage() {
  ...
  const { data, isLoading } = useQuery<Event[]>({
    ...
    queryFn: async () => {
      const res = await eventsApi.list({ venue_id: effectiveVenueId, skip: page * PAGE_SIZE, limit: PAGE_SIZE })
      return res.data
    },
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {!isLoading && filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500">
                    {effectiveVenueId ? 'No hay eventos' : 'Selecciona un venue para ver sus eventos'}
                  </td></tr>
                )}
              </tbody>
            </table>

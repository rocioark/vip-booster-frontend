...
import { ordersApi, eventsApi, ticketsApi } from '@/lib/api'
...
export default function OrdersPage() {
  ...
  // For super_admin: load events for the venue, then fetch orders per event in parallel.
  // For venue staff: the JWT auto-filters by venue, so a plain list works.
  const { data: venueEvents } = useQuery<Event[]>({
    queryKey: ['events-for-orders', effectiveVenueId],
    queryFn: async () => {
      const r = await eventsApi.list({ venue_id: effectiveVenueId, limit: 100 })
      return r.data
    },
    enabled: isSuperAdmin && !!effectiveVenueId,
  })

  const perEventQueries = useQueries({
    queries: (isSuperAdmin && venueEvents ? venueEvents : []).map(event => ({
      queryKey: ['orders-by-event', event.id],
      queryFn: async () => {
        const r = await ordersApi.list({ event_id: event.id, limit: 100 })
        return r.data as Order[]
      },
      enabled: isSuperAdmin && !!venueEvents,
    })),
  })

  const regularOrdersQuery = useQuery<Order[]>({
  ...
  const allOrders: Order[] = isSuperAdmin
    ? perEventQueries
        .flatMap(q => q.data ?? [])
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
    : (regularOrdersQuery.data ?? [])

  const isLoading = isSuperAdmin
    ? !venueEvents || perEventQueries.some(q => q.isLoading)
    : regularOrdersQuery.isLoading
  ...
  const paginatedData = isSuperAdmin ? filtered : allOrders
    ...
    <div>
      ...
      <div className="p-6 space-y-4">
        ...
        <Card>
          ...
          {!isSuperAdmin && ((paginatedData?.length ?? 0) === PAGE_SIZE || page > 0) && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              ...
              <Button variant="secondary" size="sm" disabled={(paginatedData?.length ?? 0) < PAGE_SIZE} onClick={() => setPage(p => p + 1)}>Siguiente →</Button>
            </div>
          )}
        </Card>
      </div>
      ...
      {effectiveVenueId && (
        <CreateOrderModal open={showCreate} onClose={() => setShowCreate(false)} venueId={effectiveVenueId} />
      )}
    </div>
  )
}

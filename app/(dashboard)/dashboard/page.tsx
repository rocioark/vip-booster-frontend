...
export default function DashboardPage() {
  ...
  const { data, isLoading, isError } = useQuery<VenueSummary>({
    queryKey: ['analytics-summary', user?.venue_id],
    queryFn: async () => {
      const res = await analyticsApi.summary(user?.venue_id ?? undefined)
      return res.data
    },
    enabled: !!user,
  })
      ...
      <div className="p-6 space-y-6">
              ...
              <CardBody className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900">
                    {isLoading ? <span className="inline-block h-4 w-20 bg-gray-200 rounded animate-pulse" /> : s.value}
                  </p>
                </div>
              </CardBody>
            </Card>
          ...
          <CardBody>
            {isLoading ? (
              ...
              <p className="text-3xl font-bold text-gray-900">
                {data ? fmt(data.revenue_last_30_days) : '—'}
              </p>
            )}
          </CardBody>
        </Card>
        ...
        {isError && (
          <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-sm text-red-700">
            No se pudo cargar la información del venue.
          </div>
        )}
      </div>
    </div>
